...
mfers.forEach(({ i, traits }) => {
  fs.writeFileSync(`./public/descriptions/${i}.json`,JSON.stringify({
    description: describe_mfer(traits),
    traits: traits
  },null,2))
})

function describe_mfer({ 
  type,
  shirt,
  chain,
  ['4:20 watch']:  watch,
  ...
  let phrases = {
    ...
    clothes(){
      ...
      if(wearing.length == 1){
        return `are wearing ${wearing[0]}`
      }
      if(wearing.length == 2){
        return `are wearing ${wearing[0]} and ${wearing[1]}`
      }
      if(wearing.length == 3){
        return `are wearing ${wearing[0]}, ${wearing[1]}, and ${wearing[2]}`
      }
    }
  }
